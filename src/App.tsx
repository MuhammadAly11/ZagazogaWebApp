...
function App() {
  ...
  useEffect(() => {
    ...
    const handleThemeChange = (e: MediaQueryListEvent) => {
      if (followSystem) setDarkMode(e.matches);
    };

    mediaQuery.addEventListener('change', handleThemeChange);
    if (followSystem) setDarkMode(mediaQuery.matches);
    
    darkMode ? document.documentElement.classList.add('dark') : document.documentElement.classList.remove('dark');
    
    if (!followSystem) localStorage.setItem('darkMode', String(darkMode));
  ...
  const toggleMetadataType = () => {
    setMetadata(prev => prev.type === 'lesson' ? INITIAL_CUSTOM_METADATA : INITIAL_LESSON_METADATA);
  };

  const updateLessonMetadata = (updates: Partial<LessonMetadata>) => {
    setMetadata(prev => ({ ...prev, ...updates } as LessonMetadata));
  };

  const updateCustomMetadata = (updates: Partial<CustomMetadata>) => {
    setMetadata(prev => ({ ...prev, ...updates } as CustomMetadata));
  };
  ...
  const removeTag = (index: number) => {
    if (metadata.type === 'custom') {
      setMetadata({ ...metadata, tags: metadata.tags.filter((_, i) => i !== index) });
    }
  };

  const isMetadataValid = () => {
    if (metadata.type === 'lesson') {
      return metadata.module.trim() !== '' && metadata.subject.trim() !== '' && metadata.lesson.trim() !== '';
    }
  ...
  const addQuestion = () => {
    setQuestions(prev => [
      ...prev,
      { ...INITIAL_QUESTION, sn: String(prev.length + 1) }
    ]);
  };
  ...
  const areOptionsFilledInSequence = (question: QuizQuestion, availableOptions: Option[]) => {
    for (let i = 0; i < availableOptions.length - 1; i++) {
      if (!question[availableOptions[i]] || question[availableOptions[i]].trim() === '') {
        return false;
      }
    }
    return true;
  ...
  const addExtraOption = (index: number) => {
    ...
    if (currentCount < EXTRA_OPTIONS.length && areOptionsFilledInSequence(questions[index], availableOptions)) {
      setExtraOptionsCount(prev => ({ ...prev, [index]: currentCount + 1 }));
    }
  };
  ...
  const isQuestionValid = (question: QuizQuestion) => {
    ...
    for (let i = 0; i < allOptions.length; i++) {
      ...
      if (value === '') {
        // If we find an empty option, all following must be empty
  ...
  const getQuestionValidationMessage = (question: QuizQuestion) => {
    ...
    if (hasSequenceError) {
      ...
      for (const opt of allOptions) {
        if (question[opt].trim() !== '') filledCount++;
      }
      
      if (filledCount < 2) missing.push('at least 2 options');
    }
  ...
  const downloadJSON = () => {
    ...
    const dataStr = JSON.stringify({ ...metadata, questions: cleanedQuestions }, null, 2);
    ...
    setShowPdfInfo(true);
    setTimeout(() => setShowPdfInfo(false), 8000);
  };
      ...
      {showPdfInfo && (
        <div className="fixed top-4 right-4 left-4 md:left-auto md:w-96 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-purple-100 dark:border-purple-900 p-4 animate-fade-in z-50">
          <div className="flex items-start">
            ...
            <button onClick={() => setShowPdfInfo(false)} className="flex-shrink-0 ml-2 text-gray-400 hover:text-gray-500 dark:hover:text-gray-300">
              <X className="w-5 h-5" />
            </button>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 pb-6 border-b border-gray-200 dark:border-gray-700">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="relative group">
              ...
              >
                {followSystem ? <Monitor className="w-5 h-5" /> : darkMode ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
              </button>
        ...
        <div className="space-y-8">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  <label className="block text-base font-medium text-gray-900 dark:text-white">
                    Title <span className="text-red-500 dark:text-red-400 ml-1">*</span>
                  </label>
                  <div className="relative">
                    <input
                      type="text"
                ...
                <div className="pt-4 border-t border-gray-100 dark:border-gray-700">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {/* Module Field */}
                    <div>
                      <label className="block text-xs font-medium text-gray-400 dark:text-gray-500 mb-1">Module</label>
                      <div className="relative">
                        <input
                          type="text"
                          value={metadata.module}
                          onChange={(e) => updateCustomMetadata({ module: e.target.value })}
                          className="w-full px-3 py-1.5 text-sm border border-gray-200 dark:border-gray-600 rounded-md focus:ring-1 focus:ring-purple-500 focus:border-purple-500 bg-gray-50/50 dark:bg-gray-700 dark:text-gray-300"
                    ...
                    {/* Tags Field */}
                    <div>
                      <label className="block text-xs font-medium text-gray-400 dark:text-gray-500 mb-1">Tags</label>
                      <div className="space-y-1.5">
                        {metadata.tags.length > 0 && (
                          <div className="flex flex-wrap gap-1.5">
                            {metadata.tags.map((tag, index) => (
                              <span key={index} className="inline-flex items-center px-2 py-0.5 text-xs rounded bg-gray-50 dark:bg-gray-700 text-gray-600 dark:text-gray-300 border border-gray-100 dark:border-gray-600">
                                {tag}
                                <button
          ...
          <div className="space-y-6">
            ...
            {questions.map((question, index) => (
              <div key={index} className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden p-6">
                <div className="flex justify-between items-center mb-6">
                  <div className="flex items-center space-x-3">
